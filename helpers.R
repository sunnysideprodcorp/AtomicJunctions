...
alignG <-function(alignG, trace)
{
  trace2 <- trace$Conductance - alignG
  min.index <- which.min(abs(trace2))

  offset <- trace$Displacement[min.index] 
...
point2X <-function(trace, point){
  point/(length(trace$Displacement)-1)*max(trace$Displacement)
}

#MAKE 2D HISTOGRAM
make2DHist <- function(N=50, alignG = .3, log=TRUE, method=1, createNew = TRUE, df=na)
{
  if(createNew)
  {
    v  <- replicate(N, alignG(alignG, makeTrace()), simplify=FALSE)
    df <- do.call(rbind, v)
  }
  
  if (method == 1){
    #first method uses ggplot
    d <- ggplot(df,aes(Displacement, Conductance))
